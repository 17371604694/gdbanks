...
$(document).ready(function() {
	$('#consignFileApprovalTable').bootstrapTable({
		  ...
		  //请求方法
               ...
               url: "${ctx}/consignfileapproval/confileapproval/consignFileApproval/data2",
			...
			,{
			   ...
			   formatter:  function operateFormatter(value, row, index) {
				   ...
				   if(row.state==1){
					   result1.push('<a class="del btn btn-icon waves-effect waves-light btn-success btn-xs m-r-5"> <i class="fa fa-trash-o"></i></a>')
				   }
                   if(u.id=="1" || u.id=="c6d9c07543f64a21b4454c23c8dedde2"){

                       if (u.id == row.applicantPerson) {
                           if(row.stateStep==1) {
                               result1.push('<a class="edit btn btn-icon waves-effect waves-light btn-success btn-xs m-r-5"> <i class="fa fa-pencil"></i></a>')
                           }
                           if(row.stateStep==6 && u.id == row.applicantPerson){
                               result1.push('<a class="edit btn btn-icon waves-effect waves-light btn-success btn-xs m-r-5">重新发起</a>')
                           }
					   }
 ...
 function add() {
  ...
  function edit(id,row){
    console.log(row)
	  ...
	  jp.openSaveDialog('编辑寄存档案出库', "${ctx}/consignfileapproval/confileapproval/consignFileApproval/form/edit?id="+id+"&state=1",'80%', '80%');
  }
  //查看表单页面
  function view(id) {
      if(!id){
