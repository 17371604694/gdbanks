...
$(document).ready(function() {
	$('#taskIssuedByTable').bootstrapTable({
		  ...
		  //请求方法
               ...
               url: "${ctx}/taskissuedby/taskissuedbyts/taskIssuedBy/data2",
...
// }
			...
			,{
			   ...
			   events: {
				   ...
				   'click .del': function (e, value, row, index) {
                    ...
                    'click .zx': function (e, value, row, index) {
                       zx(row.id);
			   ...
			   formatter:  function operateFormatter(value, row, index) {
                  var user = JSON.parse(jp.getCookie("user"));//用户信息
					// alert(user.id);
					// alert(row.executorId);
					// alert(row.state);
				  var views=[];
                  if(user.id==row.executorId && row.state==0){
                        views.push([
                        '<a class="zx btn btn-icon waves-effect waves-light btn-custom btn-xs m-r-5"> 处理 </a>'
                        ]);
                  }
  ...
  function zx(ids){
     ...
     jp.openSaveDialog('处理下发任务', "${ctx}/taskissuedby/taskissuedbyts/taskIssuedBy/opencl?id="+ids,'800px', '500px');

  	/*jp.get("${ctx}/taskissuedby/taskissuedbyts/taskIssuedBy/dispose?ids=" + ids, function(data){
				if(data.success){
					refresh();
					jp.toastr_success(data.msg);
				}else{
