...
$(document).ready(function() {
	$('#bankVoteTable').bootstrapTable({
		  ...
		  //请求方法
               ...
               url: "${ctx}/vote/vote/bankVote/data",
			...
			,{
			   ...
			   events: {
				   ...
				   'click .selects': function (e, value, row, index) {
					   selects(row.id);
				   },
			   ...
			   formatter:  function operateFormatter(value, row, index) {
				   return [
			   		...
			   		<% if(shiro.hasPermission("vote:vote:bankVote:view")){ %>
					   '<a class="selects btn btn-icon waves-effect waves-light btn-custom btn-xs m-r-5"> 选项列表</i></a>',
				   <% } %>
				   <% if(shiro.hasPermission("vote:vote:bankVote:edit")){ %>
					   '<a class="edit btn btn-icon waves-effect waves-light btn-success btn-xs m-r-5">编辑主题</a>',
				   <% } %>
				   <% if(shiro.hasPermission("vote:vote:bankVote:del")){ %>
					   '<a class="del btn btn-icon waves-effect waves-light btn-danger btn-xs m-r-5"> 删除主题</a>',
				   <% } %>
					   '<a class="xq btn btn-icon waves-effect waves-light btn-danger btn-xs"><i class="fa"></i>投票详情</a>'
  ...
  function del(ids){
	 ...
	 jp.confirm('确认要删除该投票记录吗？', function(){
		...
		jp.get("${ctx}/vote/vote/bankVote/delete?ids=" + ids, function(data){
				if(data.success){
					refresh();
					jp.toastr_success(data.msg);
				}else{
  ...
  function refresh() {
   ...
   //新增表单页面
 function add() {
     jp.openSaveDialog('新增投票', "${ctx}/vote/vote/bankVote/form/add",'800px', '500px');
 }
  //编辑表单页面
  function edit(id){
      if(!id){
	  ...
	  jp.openSaveDialog('编辑投票', "${ctx}/vote/vote/bankVote/form/edit?id="+id,'800px', '500px');
  }
  //添加选项表单页面
  function view(id) {
      if(!id){
	...
	function xq(id) {
		...
		jp.openViewDialog('详情', "${ctx}/vote/vote/bankVote/voteinfo?id="+id,'1300px', '700px');
	}
...
function selects(id) {
	...
	jp.openViewDialog('选项列表', "${ctx}/vote/vote/bankVote/bankSelectList?id="+id,'1000px', '500px');
}
</script>