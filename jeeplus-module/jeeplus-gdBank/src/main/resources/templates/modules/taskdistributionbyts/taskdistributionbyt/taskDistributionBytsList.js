...
$(document).ready(function() {
	$('#taskDistributionBytsTable').bootstrapTable({
		  ...
		  //请求方法
               ...
               url: "${ctx}/taskdistributionbyts/taskdistributionbyt/taskDistributionByts/data",
			...
			,{
			   ...
			   formatter:  function operateFormatter(value, row, index) {
				  ...
				  ret.push('<a class="view btn btn-icon waves-effect waves-light btn-custom btn-xs m-r-5"> <i class="fa fa-search"></i></a>')
				   if(row.state!=-1 && user.id==row.publisherId){
                       ret.push('<a class="edit btn btn-icon waves-effect waves-light btn-success btn-xs m-r-5"> <i class="fa fa-pencil"></i></a>')

				   }
				   if (user.id==row.publisherId) {
                       ret.push('<a class="del btn btn-icon waves-effect waves-light btn-danger btn-xs"> <i class="fa fa-trash-o"></a>')
				   }
