...
public class FileController extends BaseController {
	...
	public void download(@Param("source") String source, HttpServletRequest request, HttpServletResponse response) throws Exception{
		...
		if (file == null || !file.exists()) {
			...
			request.getRequestDispatcher("/webpage/error/404.html").forward(request, response);
		}
...
//			response.setHeader("Content-Disposition", "attachment; filename=" + file.getName());
	...
	public AjaxJson getUrl(@Param("dir") String dir) throws IllegalStateException, IOException {
		...
		String url = FileKit.transDirToUrl(dir);
	...
	public AjaxJson webupload( HttpServletRequest request, HttpServletResponse response,MultipartFile file) throws IllegalStateException, IOException {
		...
		SystemAuthorizingRealm.Principal principal = (SystemAuthorizingRealm.Principal) UserUtils.getPrincipal();
		String timeOny=new Date().getTime()+"";
		String fileUrl = FileKit.getAttachmentUrl()+uploadPath+"/"+year+"/"+month+"/"+timeOny+"/";
		String fileDir = FileKit.getAttachmentDir()+uploadPath+"/"+year+"/"+month+"/"+timeOny+"/";
		// 判断文件是否为空
		if (!file.isEmpty()) {
			// 文件保存路径
			// 转存文件
			...
			if(name.contains("\\")){
				...
				System.out.println("截取后的文件名："+name);
				System.out.println("IE浏览器上传文件，包含完整路径");
			}
			if(name.contains(" ")){
				 name = name.replaceAll(" ","");
			}
			String filePath = fileDir +  name;
	//		String filePath = "src/main/resources/static/upload/" +  name;
	...
	@ResponseBody
	@RequiresPermissions("user")
	@RequestMapping(value = "webupload/deleteByUrl")
