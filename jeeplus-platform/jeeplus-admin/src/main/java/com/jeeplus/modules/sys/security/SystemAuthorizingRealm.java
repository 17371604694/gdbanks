...
//			Session session = UserUtils.getSession();
		...
		// 校验用户名密码
		//登录失败超过5次锁定账号
		Object countStr = session.getAttribute(token.getUsername());
		int count =  0;
		if(!"".equals(countStr) && null!=countStr){
			count =  Integer.parseInt(countStr.toString());
		}
//		User user = getSystemService().getUserByLoginName(token.getUsername());//优先在缓存中获取用户信息
		User user = getSystemService().getUserByLoginName(token.getUsername(),"select");//去数据库查询用户信息
		if (user != null) {
			if (JeePlusProperites.NO.equals(user.getLoginFlag())){
//				session.setAttribute(token.getUsername(),6);
				session.removeAttribute(token.getUsername());
				throw new AuthenticationException("msg:该帐号已禁止登录.");
			}
			//登录5次未通过 禁用账户
			if(count>5){
				user.setLoginFlag(JeePlusProperites.NO);
				systemService.updateUserLookByPasswork(user);//禁用
//				session.setAttribute(token.getUsername(),6);
				session.removeAttribute(token.getUsername());
				throw new AuthenticationException("msg:密码错误次数超限,账号已冻结,请联系管理员.");
			}
