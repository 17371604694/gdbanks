...
public class SystemService extends BaseService  {
	 ...
	 * @return
	 */
	public User getUserByLoginName(String loginName,String select) {
		User user = userMapper.getByLoginName(new User(null, loginName));
		if (user == null){
			return null;
		}
		UserUtils.clearCache(user);//清楚当前用户缓存，重新存放
...
//		systemRealm.clearAllCachedAuthorizationInfo();
	...
	public Role getOneRoleByName(String name) {
		Role r = new Role();
		r.setName(name);
		return roleMapper.getOneRoleByName(r);
	}
...
//		systemRealm.clearAllCachedAuthorizationInfo();
	...
	public List<Menu> programaTreeData(){
		return menuMapper.programaTreeData();
	}
	...
	public List<Menu> getbusinessChildren(String parentId){
		return menuMapper.getbusinessChildren(parentId);
	}
...
//		systemRealm.clearAllCachedAuthorizationInfo();
	...
	@Transactional(readOnly = false)
    public void updateUserLookByPasswork(User user) {
		UserUtils.clearCache(user);//清除用户缓存
		userMapper.updateUserLookByPasswork(user);
    }
}
