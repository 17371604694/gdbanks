<script>
    ...
    $('#bankConferenceRoomReservationTable').bootstrapTable({
        ...
        url: "${ctx}/meetingroommanage/meetingroomconvention/meetingnotice/data?isNoticeMeeting=1",
        ...
        columns: [{
            ...
            // }
            // ,{
            //     field: 'conferenceContactPhone',
            //     title: '联系电话',
            // sortable: true,
            // sortName: 'conferenceContactPhone'
  ...
  function del(ids){
	 ...
	 jp.confirm('确认要取消该会议通知吗？取消之后将不再保留！', function(){
		var index =jp.loading();
		jp.get("${ctx}/meetingroommanage/meetingroomconvention/meetingnotice/delete?ids=" + ids, function(data){
				if(data.success){
					refresh();
					jp.toastr_success(data.msg);
				}else{
  ...
  function refresh() {
   ...
   //新增表单页面
 function add() {
     jp.openSaveDialog('新增会议通知', "${ctx}/meetingroommanage/meetingroomconvention/meetingnotice/form/add",'1200px', '600px');
 }
  //编辑表单页面
  function edit(id){
      if(!id){
	  ...
	  jp.openSaveDialog('编辑会议通知', "${ctx}/meetingroommanage/meetingroomconvention/meetingnotice/form/edit?id="+id,'800px', '500px');
  }
  //查看表单页面
  function view(id) {
      if(!id){
      ...
      jp.openViewDialog('查看会议通知', "${ctx}/meetingroommanage/meetingroomconvention/meetingnotice/form/view?id="+id,'800px', '500px');
  }
</script>