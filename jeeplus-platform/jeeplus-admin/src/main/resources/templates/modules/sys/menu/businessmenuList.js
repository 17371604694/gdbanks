...
$(document).ready(function(){  
       $treeTable=$('#treeTable').treeTable({
    	   theme:'vsStyle',	           
			...
			column:0,
			checkbox: false,
            url:'${ctx}/sys/menu/getbusinessChildren?parentId=',
...
<script type="text/html" id="menuItemTpl">
	...
	<% if(shiro.hasPermission("sys:menu:edit")) { %>
		<a href="#" onclick="jp.openSaveDialog('修改栏目', '${ctx}/sys/menu/programaform?id={{d.id}}','800px', '510px')"><i class="{{d.icon}}"></i> {{d.name}}</a>
	<% }else if(shiro.hasPermission("sys:menu:view")) { %>
		<a href="#" onclick="jp.openViewDialog('查看栏目', '${ctx}/sys/menu/programaform?id={{d.id}}','800px', '510px')"><i class="{{d.icon}}"></i> {{d.name}}</a>
    <% }else{%>
	...
	<td>
		<div class="btn-group dropdown">
				...
				<li><a href="#"  class="dropdown-item" onclick="jp.openViewDialog('查看栏目', '${ctx}/sys/menu/programaform?id={{d.id}}','800px', '600px')"><i class="fa fa-search-plus"></i> 查看</a></li>
					<% }%>
				<% if(shiro.hasPermission("sys:menu:edit")) { %>
				<li><a href="#"   class="dropdown-item" onclick="jp.openSaveDialog('修改栏目', '${ctx}/sys/menu/programaform?id={{d.id}}','800px', '600px')"><i class="fa fa-edit"></i> 修改</a></li>
					<% }%>
				...
				<li><a href="#"  class="dropdown-item" onclick="jp.openSaveDialog('添加下级栏目', '${ctx}/sys/menu/programaform?parent.id={{d.id}}','800px', '600px')"><i class="fa fa-plus"></i> 添加下级栏目</a></li>
					<% }%>
				</ul>
				</div>
		</div>
