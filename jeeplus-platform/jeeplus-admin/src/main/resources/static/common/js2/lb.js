...
function Lb(options) {
  ...
  // 暂停轮播 pause()
    Lb.prototype.pause=function() {
     ...
     // 处理点击控件事件
     Lb.prototype.clickCtrl=function (event) {
     ...
     // 处理点击标志事件
    Lb.prototype.clickSign=function (event) {
         if (!this.status) return;
         this.status = false;
         var fromIndex = this.curIndex;
     ...
     // 处理滑动屏幕事件
    Lb.prototype.touchScreen=function (event) {
     ...
     // 处理键盘按下事件
    Lb.prototype.keyDown=function (event) {
    ...
    Lb.prototype.handleEvents=function () {
      ...
      */
     Lb.prototype.slide=function (fromIndex, toIndex, direction)
     {
         if (direction == 'left') {
